...
def main():
        ...
        for sym, data in sorted(filtered_stocks.items()):
            try:
                ...
                if len(buy_sigs) > 0:
                    ...
                    picks.append((sym, latest_sig, n_sigs))
            except Exception:
                pass
    ...
    for sname, picks in all_picks.items():
        recent = [(s, d, n) for s, d, n in picks if d >= "2026-05-21"]
        print(f"\n--- {sname}: {len(recent)} stocks ---")
        if recent:
            for sym, d, n in recent[:25]:
                print(f"  {sym}  last={d}  total_signals={n}")
    ...
    rows = []
    for sname, picks in all_picks.items():
        for sym, d, n in picks:
            if d >= "2026-05-21":
                rows.append({"strategy": sname, "symbol": sym, "last_signal": d, "total_signals": n})
    if rows:
