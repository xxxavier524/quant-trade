...
def top_tax_cut(nav_peak_gain: float, tax_rate: float = 0.3, cap: float = 0.5) -> float:
    """净值相对本轮起点的浮盈比例 → 建议计提减仓比例（浮盈部分的 tax_rate）。

    例：浮盈 40%、税率 30% → 建议减掉总仓位的约 40%*30% = 12%。
    浮盈 ≤0 → 0；按体系「计提20%~50%」上限封顶 cap（默认50%）。
    """
    return min(cap, max(0.0, nav_peak_gain) * tax_rate)
