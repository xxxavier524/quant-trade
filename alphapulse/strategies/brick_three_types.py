"""AlphaPulse-A 砖型图3子类型策略 -- 横盘检测增强版（numpy 优化版）。
...
性能优化（v2）：
- SMA 内部使用 numpy 数组替代 pandas .iloc 逐行访问
- 信号检测全部使用 numpy 数组，消除 pandas 索引开销
- 合并 volume.rolling(20) 重复计算
- 短数据（<250条）跳过 bull_bear_line 计算（全部为 NaN）
- 主循环中快速跳过非红砖日
"""
...
def detect_consolidation(data, lookback=5, max_amplitude=15.0,
                         ma20_vol=None) -> pd.Series:
    ...
    Args:
        ...
        ma20_vol: 预计算的20日均量，避免重复 rolling(20)
...
def _find_green_to_red_transition(
    is_green: np.ndarray, is_red: np.ndarray, end: int, lookback: int = 5
) -> int:
    """在前 lookback 日内寻找最近的绿→红转换点。

    Args:
        is_green: indicator==0 布尔数组
        is_red: indicator>0 布尔数组
        end: 搜索终点索引（不含）
        lookback: 回看天数
    ...
    for j in range(end - 1, start, -1):
        if is_green[j - 1] and is_red[j]:
            return j
    return -1


def _recent_consolidation(
    consolidation: np.ndarray, end: int, lookback: int
) -> bool:
    """检查 [end-lookback, end) 窗口内是否存在横盘。

    Args:
        consolidation: detect_consolidation 输出的 bool 数组
        end: 窗口终点索引（不含）
        lookback: 回看天数
    ...
    if end - start < 2:
        return False
    return bool(consolidation[start:end].any())
...
def _check_n_jump(
    ...
    """检查 Type 1: N型起跳 (BRICK_N_JUMP)。numpy 数组版本。
    ...
    if require_consolidation and not consol_detected:
        return (0.0, False)
    ...
    for j in range(transition_idx - 1, max(0, transition_idx - 20), -1):
        if is_red[j]:
            prev_red_val = brick[j]
            break
    if prev_red_val > 0 and brick[i] <= prev_red_val * 2 / 3:
        return (0.0, consol_detected)

    # ---- 条件4: 成交量 > 20日均量 × vol_mult ----
    vol_ma = ma20_vol[i]
    if np.isnan(vol_ma) or volume[i] <= vol_ma * vol_mult:
        return (0.0, consol_detected)
    ...
    if np.isnan(bb) or bb <= 0:
        return (0.0, consol_detected)
    if abs(close[i] - bb) / bb > tolerance:
        return (0.0, consol_detected)
...
def _check_continuation(
    ...
    """检查 Type 2: 上涨中继 (BRICK_CONTINUATION)。numpy 数组版本。
    ...
    for pd_len in range(pd_lo, pd_hi + 1):
        if i - pd_len < 0:
            continue
        if is_green[i - pd_len:i].all() and is_red[i]:
            pullback_len = pd_len
            break
    ...
    red_count = is_red[trend_start:trend_end].sum()
    if red_count < max(2, window_len * 0.35):
        return (0.0, False)
    # 趋势末端（最后3个）至少1个红砖
    tail_start = max(trend_start, trend_end - 3)
    if is_red[tail_start:trend_end].sum() < 1:
        return (0.0, False)

    # ---- 条件4: 回调最低价 > 最近放量阳线收盘价（不破位） ----
    last_hvb_close = None
    for j in range(trend_end - 1, max(0, trend_start - 5), -1):
        if high_vol_bullish[j]:
            last_hvb_close = close[j]
            break

    if last_hvb_close is not None and last_hvb_close > 0:
        pullback_low = low[i - pullback_len:i].min()
        if pullback_low <= last_hvb_close:
            return (0.0, False)

    # ---- 条件5 [NEW]: 横盘不深 ----
    # 回调深度 = (趋势高点 - 回调最低) / 趋势高点
    trend_high = high[trend_start:trend_end].max()
    pullback_low_val = low[i - pullback_len:i].min()
    if trend_high > 0:
    ...
    if i - pullback_len >= 0:
        consol_in_pullback = bool(
            consolidation[max(0, i - pullback_len):i].any()
        )
    ...
    if volume[i] <= volume[i - 1] * vol_expand_mult:
        return (0.0, False)
    ...
    vol_exp = volume[i] / max(volume[i - 1], 1)
    if vol_exp > 2.0:
        confidence += 0.15
    elif vol_exp > 1.5:
        confidence += 0.10
...
def _check_breakout(
    ...
    """检查 Type 3: 横盘突破 (BRICK_BREAKOUT)。numpy 数组版本。
    ...
    # ---- 条件1b: 振幅验证 ----
    cons_high = high[cons_start:i].max()
    cons_low = low[cons_start:i].min()
    if cons_low <= 0:
        return (0.0, True)
    ...
    # ---- 条件2+3: 当日红砖突破横盘上沿 ----
    if close[i] <= cons_high:
        return (0.0, True)

    # 突破前高（横盘前20日）
    pre_cons_start = max(0, cons_start - 20)
    pre_high = high[pre_cons_start:cons_start].max()
    if pre_high > 0 and close[i] <= pre_high:
        return (0.0, True)
    ...
    # ---- 条件5: 横盘期间红砖 >= 绿砖 ----
    red_count = is_red[cons_start:i].sum()
    green_count = is_green[cons_start:i].sum()
    ...
    # 突破力度
    breakout_pct = (close[i] - cons_high) / cons_high
...
def generate_signals(
    ...
    **params,
    ...
    # 短数据跳过 bull_bear_line（MA(250)全部为 NaN，Type1 无法触发）
    if n_data >= 250:
        bull_bear_line = compute_bull_bear_line(close)
    else:
        bull_bear_line = pd.Series(np.nan, index=data.index)

    # 横盘检测（传入预计算的 ma20_vol，避免重复 rolling(20)）
    consolidation = detect_consolidation(
        data, lookback=consol_lookback, max_amplitude=consol_max_amp,
        ma20_vol=ma20_vol,
    )

    # ---- 转 numpy 数组（信号检测全程 numpy，零 pandas 开销） ----
    brick_np = brick.values
    ma20_vol_np = ma20_vol.values
    ...
    # 放量阳线
    high_vol_bullish_np = (
        (close_np > open_np) & (volume_np > ma20_vol_np * 1.5)
    )
    ...
    for i in range(min_idx, n_data):
        ...
        c1, cd1 = _check_n_jump(
            brick_np, is_red_np, is_green_np, volume_np, ma20_vol_np,
            bull_bear_np, close_np, consolidation_np, i,
        ...
        c2, cd2 = _check_continuation(
            brick_np, is_red_np, is_green_np, volume_np, close_np,
            low_np, high_np, high_vol_bullish_np, consolidation_np, i,
        ...
        c3, cd3 = _check_breakout(
            brick_np, is_red_np, is_green_np, volume_np, ma20_vol_np,
            close_np, high_np, low_np, consolidation_np, i,
        ...
        has_n_jump = False
        has_cont = False
        for bt, _, _ in signals_today:
            if bt == "BRICK_N_JUMP":
                has_n_jump = True
            elif bt == "BRICK_CONTINUATION":
                has_cont = True
        ...
        for brick_type, confidence, consol_detected in signals_today:
            results.append({
                ...
                "factor_snapshot": {
                    ...
                    "confidence": round(confidence, 2),
                    "consolidation_detected": bool(consol_detected),
                    "close": round(float(close_np[i]), 2),
                    "volume": float(volume_np[i]),
                    "vol_ma20": (
                        round(float(ma20_vol_np[i]), 0)
                        if not np.isnan(ma20_vol_np[i]) else None
                    ),
                    "bull_bear": (
                        round(float(bull_bear_np[i]), 2)
                        if not np.isnan(bull_bear_np[i]) else None
                    ),
                },
            })
